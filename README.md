...
> [!NOTE]
> **What this repo demonstrates at a glance**
> - **Two-stage ML pipeline** — LightGBM classifier + per-target regressor, trained end-to-end on real German body-shop data
> - **Tested REST API** — FastAPI service (`/health`, `/predict`, `/model-info`), 23 integration tests, Pydantic v2 schemas, degraded-mode startup
> - **Containerised serving** — multi-stage Dockerfile + Compose, `make docker-serve` starts the API in one command
> - **Champion-challenger lifecycle** — explicit 3-rule promotion logic, dry-run support, structured JSON + Markdown decision reports
> - **CI on Python 3.11 + 3.12** — lint → generate → train → predict → API tests on every push
> - **Monitoring awareness** — post-deployment quality snapshot generated by `make monitor`
