...
class RepairOrderEDA:
    """Class-based Exploratory Data Analysis module for Repair Orders."""
    
    def __init__(self, data_path: Path, plot_dir: Path):
    ...
    def load_data(self):
        ...
        n_records = len(self.raw)
        print(f"\n  Total records: {n_records:,}")
        print(f"  File size    : {self.data_path.stat().st_size / 1024 / 1024:.2f} MB")

        # Check structural integrity
        missing_input        = sum(1 for r in self.raw if "input"  not in r)
        missing_output       = sum(1 for r in self.raw if "output" not in r)
        missing_positions    = sum(1 for r in self.raw
                                   if "input" in r and
                                   "calculatedPositions" not in r.get("input", {}))
        missing_make         = sum(1 for r in self.raw
        ...
        for t in OUTPUT_TARGETS:
            c = sum(1 for r in self.raw if "output" in r and t not in r["output"])
            if c:
        ...
        self.valid   = [r for r in self.raw if "input" in r and "output" in r
                   and "calculatedPositions" in r["input"]]
        self.invalid_count = n_records - len(self.valid)
        print(f"\n  Valid records (both input + output): {len(self.valid):,}")
        if self.invalid_count:
            print(f"  [WARN] Skipped {self.invalid_count} malformed records")

    def analyze_target_sparsity(self):
        section("STEP 2 — Target Variable Analysis")

        self.df_targets = pd.DataFrame([r["output"] for r in self.valid])[OUTPUT_TARGETS].fillna(0.0)
        ...
        sparsity = {}
        for t in OUTPUT_TARGETS:
            n_zero    = (self.df_targets[t] == 0).sum()
            n_nonzero = (self.df_targets[t] >  0).sum()
            sparsity[t] = {
                "n_zero":    n_zero,
                "n_nonzero": n_nonzero,
                "pct_zero":  100 * n_zero / len(self.df_targets),
            }

        sdf = pd.DataFrame(sparsity).T.sort_values("pct_zero", ascending=False)
        self.sdf_cache = sdf # cached for summary step
        ...
        plt.tight_layout()
        self._save_fig("01_target_sparsity")
        ...
        nonzero_stats = {}
        for t in OUTPUT_TARGETS:
            vals = self.df_targets.loc[self.df_targets[t] > 0, t]
            if len(vals) < 2:
                continue
            nonzero_stats[t] = {
                "count":  len(vals),
        ...
        plt.tight_layout()
        self._save_fig("02_target_distributions")

        # ── 2c. Target co-occurrence heatmap ─────────────────────────────────────────
        subsection("2c. Target Co-Occurrence")

        self.binary_targets = (self.df_targets > 0).astype(int)
        cooc   = self.binary_targets.T.dot(self.binary_targets)
        ...
        plt.tight_layout()
        self._save_fig("03_target_cooccurrence")

        # ── 2d. Active targets per order ─────────────────────────────────────────────
        subsection("2d. Repair Complexity — Active Targets per Order")

        self.n_active = self.binary_targets.sum(axis=1)
        print("\n  Active work steps per order:")
        print(f"    Mean   : {self.n_active.mean():.2f}")
        print(f"    Median : {self.n_active.median():.1f}")
        print(f"    Max    : {self.n_active.max()}")
        print(f"    Min    : {self.n_active.min()}")
        print("\n  Distribution of active step counts:")
        print(self.n_active.value_counts().sort_index().to_string())

        fig, ax = plt.subplots(figsize=(8, 4))
        sns.histplot(self.n_active, bins=range(0, int(self.n_active.max()) + 2), ax=ax,
        ...
        plt.tight_layout()
        self._save_fig("04_active_targets_per_order")

        # ── 2e. Total predicted time per order ───────────────────────────────────────
        self.total_time = self.df_targets.sum(axis=1)
        subsection("2e. Total Predicted Time per Order")
        print("\n  Total predicted time (sum of all targets) per order:")
        print(f"    Mean   : {self.total_time.mean():.2f} hrs")
        print(f"    Median : {self.total_time.median():.2f} hrs")
        print(f"    Max    : {self.total_time.max():.2f} hrs")
        print(f"    Min    : {self.total_time.min():.2f} hrs")

        fig, ax = plt.subplots(figsize=(8, 4))
        sns.histplot(self.total_time[self.total_time > 0], bins=50, kde=True, ax=ax, color=COLORS[1])
        ax.set_xlabel("Total Predicted Hours per Order")
        ax.set_title("Distribution of Total Predicted Repair Time")
        plt.tight_layout()
        self._save_fig("05_total_predicted_time")

    def audit_input_quality(self):
        section("STEP 3 — Input Field Quality Audit")

        # Flatten all positions into one dataframe
        pos_rows = []
        for idx, r in enumerate(self.valid):
        ...
        self.df_pos = pd.DataFrame(pos_rows)
        print(f"\n  Total positions (all orders combined): {len(self.df_pos):,}")

        # ── 3a. Make ─────────────────────────────────────────────────────────────────
        subsection("3a. Make / Brand Distribution")
        self.makes = pd.Series([r["input"].get("make", None) for r in self.valid])
        make_counts = self.makes.value_counts(dropna=False)
        print(f"\n  Top makes:\n{make_counts.head(20).to_string()}")
        print(f"\n  Unknown / null makes : {(self.makes.isin(['unknown', 'Unknown', None])).sum()}")
        ...
        plt.tight_layout()
        self._save_fig("06_make_distribution")

        # ── 3b. Positions per order ───────────────────────────────────────────────────
        subsection("3b. Positions per Order")
        self.pos_per_order = self.df_pos.groupby("order_idx").size()
        print("\n  Positions per order:")
        print(f"    Mean   : {self.pos_per_order.mean():.1f}")
        print(f"    Median : {self.pos_per_order.median():.0f}")
        print(f"    Min    : {self.pos_per_order.min()}")
        print(f"    Max    : {self.pos_per_order.max()}")

        fig, ax = plt.subplots(figsize=(8, 4))
        sns.histplot(self.pos_per_order, bins=40, kde=True, ax=ax, color=COLORS[2])
        ax.set_xlabel("Positions per Order")
        ax.set_title("Distribution of Line-Item Count per Repair Order")
        plt.tight_layout()
        self._save_fig("07_positions_per_order")

        # ── 3c. Position type breakdown ───────────────────────────────────────────────
        subsection("3c. Position Type Breakdown")
        n_empty_text  = (self.df_pos["text"].str.strip() == "").sum()
        n_header_row  = ((self.df_pos["totalPrice"] == 0) & (self.df_pos["totalTime"] == 0)
                         & (self.df_pos["text"].str.strip() != "")).sum()
        n_useful      = ((self.df_pos["totalPrice"] > 0) | (self.df_pos["totalTime"] > 0)).sum()
        n_null_cc     = self.df_pos["genericCostCenter"].isna().sum()

        print("\n  Position breakdown:")
        print(f"    Empty text rows              : {n_empty_text:,}  ({100*n_empty_text/len(self.df_pos):.1f}%)")
        print(f"    Header/separator rows (0,0) : {n_header_row:,}  ({100*n_header_row/len(self.df_pos):.1f}%)")
        print(f"    Rows with price or time > 0 : {n_useful:,}  ({100*n_useful/len(self.df_pos):.1f}%)")
        print(f"    Null genericCostCenter       : {n_null_cc:,}  ({100*n_null_cc/len(self.df_pos):.1f}%)")

        # ── 3d. Cost center distribution ─────────────────────────────────────────────
        subsection("3d. Cost Center Distribution")
        cc_counts = self.df_pos["genericCostCenter"].value_counts(dropna=False)
        print(f"\n{cc_counts.to_string()}")
        ...
        cc_time = self.df_pos[self.df_pos["totalTime"] > 0].groupby("genericCostCenter")["totalTime"].sum()
        ...
        plt.tight_layout()
        self._save_fig("08_cost_center_distribution")

        # ── 3e. Text quality ─────────────────────────────────────────────────────────
        subsection("3e. Text Quality")

        useful_text = self.df_pos.loc[
            (self.df_pos["text"].str.strip() != "") &
            ((self.df_pos["totalPrice"] > 0) | (self.df_pos["totalTime"] > 0)),
            "text"
        ]
        ...
        plt.tight_layout()
        self._save_fig("09_top_tokens")

    def verify_numeric_consistency(self):
        section("STEP 4 — Numeric Consistency & Unit Verification")

        subsection("4a. Price Distribution (non-zero positions)")
        nonzero_price = self.df_pos.loc[self.df_pos["totalPrice"] > 0, "totalPrice"]
        ...
        neg_price = (self.df_pos["totalPrice"] < 0).sum()
        print(f"    Negative prices : {neg_price}")

        subsection("4b. Input Time Distribution (non-zero positions)")
        nonzero_time = self.df_pos.loc[self.df_pos["totalTime"] > 0, "totalTime"]
        ...
        print("        typical AW=6 minutes → divide by 10 to get hours")
        print("        If values like 300 = 5h → 1AW = 1 minute → divide by 60")
        ...
        for cc, targets in CC_TO_TARGET.items():
            if not targets:
                continue
            for t in targets:
                in_time = (self.df_pos[self.df_pos["genericCostCenter"] == cc]
                           .groupby("order_idx")["totalTime"].sum())
                out_time = self.df_targets[t]
        ...
        cc_list = [cc for cc in ["bodywork","painting","paintmaterial","material","others"]
                   if cc in self.df_pos["genericCostCenter"].values]
        axes = axes.flatten()
        for i, cc in enumerate(cc_list[:6]):
            sub = self.df_pos[(self.df_pos["genericCostCenter"] == cc) &
                         (self.df_pos["totalPrice"] > 0) & (self.df_pos["totalTime"] > 0)]
            ax = axes[i]
        ...
        plt.tight_layout()
        self._save_fig("10_price_vs_time_by_cc")

        subsection("4d. Input Painting Time vs Output Painting Targets")
        paint_in = (self.df_pos[self.df_pos["genericCostCenter"] == "painting"]
                    .groupby("order_idx")["totalTime"].sum())
        ...
        for i, t in enumerate(["paintingPreparation", "paintingSpraying", "paintingFinish"]):
            merged = pd.DataFrame({"input_paintTime": paint_in,
                                   "output": self.df_targets[t]}).dropna()
        ...
        plt.tight_layout()
        self._save_fig("11_painting_input_vs_output")

    def assess_feature_engineering_candidates(self):
        ...
        self.df_feats = pd.DataFrame(order_feats).set_index("order_idx")
        numeric_feats = [c for c in self.df_feats.columns if c != "make"]
        df_feats_num  = self.df_feats[numeric_feats].astype(float)

        subsection("5a. Input Feature × Output Target Correlation")
        corr_list = []
        for feat_col in numeric_feats:
            for t in OUTPUT_TARGETS:
                mask = self.df_targets[t] > 0
                if mask.sum() < 10:
                    continue
                r, p = stats.spearmanr(df_feats_num.loc[mask.values, feat_col],
                                        self.df_targets.loc[mask.values, t])
        ...
        plt.yticks(fontsize=8)
        plt.tight_layout()
        self._save_fig("12_feature_target_correlation")

        subsection("5b. Make vs Key Targets")
        df_full = self.df_feats[["make"]].copy()
        df_full = df_full.join(self.df_targets)
        top_makes_list = self.makes.value_counts().head(8).index.tolist()
        ...
        plt.suptitle("Work Step Duration by Vehicle Make (non-zero records, no outliers)",
                     y=1.01, fontsize=11)
        plt.tight_layout()
        self._save_fig("13_make_vs_targets")
    ...
    def evaluate_baseline(self):
        ...
        for factor in [6, 10, 12, 60, 1]:
            y_preds, y_trues = [], []
            occ_preds, occ_trues = [], []
            for i, r in enumerate(self.valid):
                p = rule_predict(r, input_time_conversion=factor)
                for t in OUTPUT_TARGETS:
                    y_preds.append(p.get(t, 0))
                    y_trues.append(self.df_targets.loc[i, t])
                    occ_preds.append(1 if p.get(t, 0) > 0 else 0)
                    occ_trues.append(1 if self.df_targets.loc[i, t] > 0 else 0)
        ...
        target_scores = {}
        for t in OUTPUT_TARGETS:
            y_pred = [rule_predict(r, best_factor).get(t, 0) for r in self.valid]
            y_true = self.df_targets[t].values
        ...
        plt.tight_layout()
        self._save_fig("14_baseline_scores")
  ...
  |  DATASET SUMMARY                                                |
  +-------------------------------------------------------------------+
  |  Records          : {len(self.raw):>8,}                                  |
  |  Valid records    : {len(self.valid):>8,}                                  |
  |  Output targets   : 14 (multi-target regression + classification)|
  |  Avg positions/order      : {self.pos_per_order.mean():>5.1f}                         |
  |  Avg active targets/order : {self.n_active.mean():>5.2f}                         |
  |  Most sparse target  : {self.sdf_cache.index[0]:<20} ({self.sdf_cache['pct_zero'].iloc[0]:.1f}% zero)   |
  |  Least sparse target : {self.sdf_cache.index[-1]:<20} ({self.sdf_cache['pct_zero'].iloc[-1]:.1f}% zero)   |
  +-------------------------------------------------------------------+
...
""")
    ...
    def run_all(self):
        """Run all typical EDA tasks sequentially."""
        self.load_data()
        self.analyze_target_sparsity()
        self.audit_input_quality()
        self.verify_numeric_consistency()
        self.assess_feature_engineering_candidates()
...
def main():
    ...
    plot_dir = root / "docs" / "assets" / "eda_plots"

    eda = RepairOrderEDA(data_path, plot_dir)
    eda.run_all()
